...
?>
	...
	<div data-role="content" data-theme="i">	
			...
			<div class="ui-body ui-body-a">
				...
				<p>The Fall 2014 Season of Team Lex Volleyball schedule will be posted soon.  Matches will be two sets to 21 with a cap at 23 unless otherwise noted, using
...
<p>The first week of Team Lex is dedicated to <i>Skill Assessments</i>.  <i>Skill Assessments</i> are a collection of passing, setting, hitting, and serving drills to evaluate player abilities for team placement.  League organizers will help lead the drills and evaluate players in order to form evenly matched teams.</p>
    ...
    <thead>
		...
		<tr>
			<th>5:30pm</th>
			<td valign=top>Skills Assessment Group 1</td>
        </tr>
		<tr>
			<th>6:15pm</th>
			<td valign=top>Skills Assessment Group 2</td>
        </tr>
		<tr>
			<th>7:00pm</th>
			<td valign=top>Skills Clinic: Basics of the Game (Rules, Points, Rotation)</td>
        </tr>
		<tr>
			<th>7:30pm</th>
			<td valign=top>Finishing Remarks</td>
        </tr>
		</tbody>
		</table>
							...
							<?php if ($rowMatch[Extra1] <> ""){ ?>	
							   <center>							
								...
								<tbody>
									...
									<td valign=top><?php print $rowMatch[Extra1];?></td>
									<td valign=top><?php print $rowMatch[Extra2];?></td>
								</tr>
								</tbody>
								</table>
								</center>
								...
								if (mysql_num_rows($matchesQueryResult) == 0) {
								  print "<tr><th colspan=7 align=center><b>No matches have been scheduled yet for this week.</th>";
									...
									while ($rowMatch2 = mysql_fetch_array($matchesQueryResult, MYSQL_ASSOC)) 
										...
										<td valign=top><a href="team.php?ID=<?php print $rowMatch2[homeID]; ?>" data-theme="<?php print $rowMatch2[homeDataTheme]; ?>"><?php print $rowMatch2[homeTeam]; ?></a></td>
										<td valign=top><a href="team.php?ID=<?php print $rowMatch2[awayID]; ?>" data-theme="<?php print $rowMatch2[awayDataTheme]; ?>"><?php print $rowMatch2[awayTeam]; ?></a></td>
										<td valign=top><a href="#" data-theme="<?php print $rowMatch2[refDataTheme]; ?>"><?php print $rowMatch2[refTeam]; ?></a></td>
									...
									if ($num_rows > 0){
									  ?>
									  <td valign=top style="border-left:1px solid black"><a href="team.php?ID=<?php print $rowMatch2[homeID]; ?>" data-theme="<?php print $rowMatch2[homeDataTheme]; ?>"><?php print $rowMatch2[homeTeam]; ?></a></td>
										<td valign=top><a href="team.php?ID=<?php print $rowMatch2[awayID]; ?>" data-theme="<?php print $rowMatch2[awayDataTheme]; ?>"><?php print $rowMatch2[awayTeam]; ?></a></td>
										<td valign=top><a href="#" data-theme="<?php print $rowMatch2[refDataTheme]; ?>"><?php print $rowMatch2[refTeam]; ?></a></td>
									  <?php
									  $num_rows= $num_rows-1;
									  ...
									  print "<td valign=top colspan=3 style='display:none'></td>";
									}
